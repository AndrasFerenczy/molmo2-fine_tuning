...
def _build_gumbel_sliding_attn_bias_BxHx2Tx2T(
    prefix_log_BxHxT: torch.Tensor,
    beacon_log_alpha_BxHxT: torch.Tensor,
    window_size: int,
    ...
    beacon_bias_2t_BxHx2T = torch.zeros((b, h, two_t), device=device, dtype=dtype)
...
def gumbel_sliding_attention(
    ...
    """
    Gumbel sliding attention reference implementation with optional document
    masking. Uses SDPA with an explicit additive mask.
    """
    del sm_scale, warp_specialize

    _, _, two_t, _ = q.shape
    assert two_t % 2 == 0, "Expected doubled sequence length (2T) for Gumbel layout."

    attn_bias_BxHx2Tx2T = _build_gumbel_sliding_attn_bias_BxHx2Tx2T(
        prefix_log_BxHxT=prefix_log_BxHxT,
        beacon_log_alpha_BxHxT=beacon_log_alpha_BxHxT,
        window_size=window_size,
        zero_normal_bias_in_window=zero_normal_bias_in_window,
    ).to(q.dtype)

    causal = torch.ones((two_t, two_t), device=q.device, dtype=torch.bool).tril()
    ...
    attn_mask = attn_bias_BxHx2Tx2T + causal_additive
